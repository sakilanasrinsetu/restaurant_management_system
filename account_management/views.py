...
# Logout All View Function
class LogoutAllView(KnoxLogoutAllView):
    """Logout all => Extending Knox"""
        ...
        if user_qs.exists():
            ...
            if is_waiter:
                StaffFcmDevice.objects.filter(
                    hotel_staff__user_id=request.user.pk
                ).delete()
            if is_customer:
                CustomerFcmDevice.objects.filter(
                    customer__user_id=request.user.pk
                ).delete()
