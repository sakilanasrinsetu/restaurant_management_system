...
class RestaurantViewSet(LoggingMixin, CustomViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        hide_sell_report = request.data.get('hide_sell_report')
        highest_price_amount_hide = request.data.get('highest_price_amount_hide')
        lowest_price_amount_hide = request.data.get('lowest_price_amount_hide')
        if hide_sell_report == 'HIGHEST_PRICE' and highest_price_amount_hide <= 0:
            return ResponseWrapper(error_msg='Highest price Amount is required', error_code=400)
        elif hide_sell_report == 'LOWEST_PRICE' and lowest_price_amount_hide <= 0:
            return ResponseWrapper(error_msg='Lowest price Amount is required', error_code=400)
...
class InvoiceViewSet(LoggingMixin, CustomViewSet):
    ...
    def get_permissions(self):
        permission_classes = []
        if self.action in ["generate_datewise_filtered_report_pdf", "generate_datewise_filtered_report_excel",'invoice_all_report']:
            permission_classes = [
                custom_permissions.IsRestaurantManagementOrAdmin
            ]
        else:
    ...
    def invoice_all_report(self, request=None, restaurant=None, getOnlyData=False, *args, **kwargs):
        ...
        if getOnlyData == True:
            ...
            # .......... Fake Report ..........
            number_of_hide_report = 0
            show_total_food_order = 0
            browsing_as_ghost = request.user.browsing_as_ghost
            if browsing_as_ghost:
                hide_sell_report = invoice_filter_qs.first().restaurant.hide_sell_report
                number_of_report_hide = invoice_filter_qs.first().restaurant.number_of_report_hide
                hide_sell_report_is_percentage = invoice_filter_qs.first().restaurant.hide_sell_report_is_percentage
                if hide_sell_report == 'HIGHEST_PRICE':
                    highest_price_amount_hide = invoice_filter_qs.first().restaurant.highest_price_amount_hide
                    invoice_filter_qs = invoice_filter_qs.exclude(payable_amount__gte = highest_price_amount_hide)

                elif hide_sell_report == 'LOWEST_PRICE':
                    lowest_price_amount_hide = invoice_filter_qs.first().restaurant.lowest_price_amount_hide
                    invoice_filter_qs = invoice_filter_qs.exclude(payable_amount__lte=lowest_price_amount_hide)

                elif hide_sell_report == 'FOOD_ORDER':
                    if hide_sell_report_is_percentage:
                        number_of_hide_report = int(number_of_report_hide*invoice_filter_qs.count()/100)
                        show_total_food_order = invoice_filter_qs.count() - number_of_hide_report
                        invoice_filter_qs = random.sample(list(invoice_filter_qs), show_total_food_order)
                    else:
                        number_of_hide_report = number_of_report_hide
                        show_total_food_order = invoice_filter_qs.count() - number_of_hide_report
                        invoice_filter_qs = random.sample(list(invoice_filter_qs), show_total_food_order)
            # .......... End Fake Report ........
...
class ReviewViewset(LoggingMixin, CustomViewSet):
    ...
    def review_list(self, request, restaurant, *args, **kwargs):
        restaurant_qs = Review.objects.filter(order__restaurant_id=restaurant).order_by('-order__created_at')
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not order:
            return ResponseWrapper(error_msg=['order id is required'], error_code=400)
        if serializer.is_valid():
            qs = Review.objects.create(order_id = order, rating = rating, review_text= review_text)
